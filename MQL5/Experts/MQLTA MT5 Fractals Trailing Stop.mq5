...
int OnInit()
{
    ...
    Symbols[0] = Symbol();
    SymbolHandles[0] = iFractals(Symbol(), PERIOD_CURRENT);
...
double GetStopLoss(string symbol, int line)
{
    ...
    if (index == -1) // Handle not found.
    {
        ...
        SymbolHandles[index] = iFractals(symbol, PERIOD_CURRENT);
    }

    double buf[];
    ArrayResize(buf, BarsToScan);
    // Copy buffer.
    int n = CopyBuffer(SymbolHandles[index], line, 0, BarsToScan, buf); // LOWER_LINE for buy trades, UPPER_LINE for sell trades.
    if (n < BarsToScan)
    {
        Print("Fractals data not ready for " + Symbols[index] + ".");
    }
    double Fractals = 0;
    int counter = 0;
...
{
    return GetStopLoss(symbol, LOWER_LINE);
}

double GetStopLossSell(string symbol)
{
    return GetStopLoss(symbol, UPPER_LINE);
}
